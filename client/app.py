...
class ClientApp(ctk.CTk):
    ...
    def show_admin_login(self):
        ...
        def submit():
            ...
            def task():
                try:
                    # Use /api/admin/verify — dedicated endpoint for admin credential check
                    r = requests.post(f"{SERVER_URL}/api/admin/verify",
                                      json={"org_id": self.org_id, "login": login, "password": password}, timeout=10)
                    if r.status_code == 200:
                        self.after(0, lambda: [dialog.destroy(),
                                               AdminPanel(self, self.org_id, login, password,
                                                          self.user_data, self.sync_total_minutes)])
                    else:
                        msg = r.json().get('error', 'Неверные данные')
                        self.after(0, lambda: error_label.configure(text=msg))
                except Exception as e:
